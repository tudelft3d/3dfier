  ...
  try {
    ...
    if (argc == 2) {
      ...
      if (s == "--version") {
        ...
        throw std::exception("Usage: 3dfier config.yml -o output.ext");
      }
    }
    ...
    if (!f.good()) {
      throw std::exception(("Configuration file " + (std::string)argv[1] + " doesn't exist").c_str());
    }
    f.close();
    ...
    if (validate_yaml(argv[1], allowedFeatures) == false) {
      throw std::exception("ERROR: config file (*.yml) is not valid. Aborting.");
    }
    ...
    if (n["extent"]) {
      ...
      if (!wentgood || xmin > xmax || ymin > ymax || boost::geometry::area(Box2(Point2(xmin, ymin), Point2(xmax, ymax))) <= 0.0) {
        throw std::exception(("ERROR: The supplied extent is not valid: (" + n["extent"].as<std::string>() + "), using all polygons").c_str());
      }
      else {
    ...
    if (!added) {
      throw std::exception("ERROR: Missing polygon data, cannot 3dfy the dataset. Aborting.");
    }
    ...
    std::exception ERR_NO_ELEV_DATA = std::exception("ERROR: Missing elevation data, cannot 3dfy the dataset. Aborting.");
    for (auto it = n.begin(); it != n.end(); ++it) {
      ...
      for (auto it2 = tmp.begin(); it2 != tmp.end(); ++it2) {
        ...
        if (path.stem() == "*") {
          if (!boost::filesystem::exists(rootPath) || !boost::filesystem::is_directory(rootPath)) {
            throw std::exception(("ERROR: " + rootPath.string() + "is not a directory, skipping it.").c_str());
          }
          else {
            ...
            for (boost::filesystem::recursive_directory_iterator it(rootPath); it != it_end; ++it) {
              if (boost::filesystem::is_regular_file(*it) && it->path().extension() == path.extension()) {
                ...
                if (!added) {
                  throw ERR_NO_ELEV_DATA;
                }
              }
            }
          }
        }
        //-- add a single file file
        else {
    ...
    if (format == "CityGML") {
      ...
      if (map3d.get_shapefile(outputFilename)) {
        ...
        throw std::exception("Writing shapefile failed");
      }
    }
    outputfile.close();
  }
...
bool validate_yaml(const char* arg, std::set<std::string>& allowedFeatures) {
    ...
    if ((*it)["lifting_per_layer"]) {
      ...
      for (auto it2 = tmp.begin(); it2 != tmp.end(); ++it2) {
        if (allowedFeatures.count((it2->second).as<std::string>()) == 0) {
          throw std::exception(("\tLifting class '" + (it2->second).as<std::string>() + "' unknown.").c_str());
        }
      }
    }
    else if ((*it)["lifting"]) {
      ...
      for (auto it2 = tmp.begin(); it2 != tmp.end(); ++it2) {
        if (allowedFeatures.count((*it)["lifting"].as<std::string>()) == 0) {
          throw std::exception(("\tLifting class '" + (*it)["lifting"].as<std::string>() + "' unknown.").c_str());
        }
      }
    }
  }
  ...
  if (n["Building"]) {
    if (n["Building"]["height_roof"]) {
      ...
      if ((s.substr(0, s.find_first_of("-")) != "percentile") ||
        ...
        throw std::exception("\tOption 'Building.height_roof' invalid; must be 'percentile-XX'.");
      }
    }
    if (n["Building"]["height_ground"]) {
      ...
      if ((s.substr(0, s.find_first_of("-")) != "percentile") ||
        (is_string_integer(s.substr(s.find_first_of("-") + 1), 0, 100) == false)) {
        throw std::exception("\tOption 'Building.height_ground' invalid; must be 'percentile-XX'.");
      }
    }
    if (n["Building"]["lod"]) {
      if (is_string_integer(n["Building"]["lod"].as<std::string>(), 0, 1) == false) {
        throw std::exception("\tOption 'Building.lod' invalid; must be an integer between 0 and 1.");
      }
    }
    if (n["Building"]["triangulate"]) {
      std::string s = n["Building"]["triangulate"].as<std::string>();
      if ((s != "true") && (s != "false")) {
        throw std::exception("\tOption 'Building.triangulate' invalid; must be 'true' or 'false'.");
      }
    }
  }
  if (n["Water"]) {
    if (n["Water"]["height"]) {
      ...
      if ((s.substr(0, s.find_first_of("-")) != "percentile") ||
        (is_string_integer(s.substr(s.find_first_of("-") + 1), 0, 100) == false)) {
        throw std::exception("\tOption 'Water.height' invalid; must be 'percentile-XX'.");
      }
    }
  }
  if (n["Road"]) {
    if (n["Road"]["height"]) {
      ...
      if ((s.substr(0, s.find_first_of("-")) != "percentile") ||
        (is_string_integer(s.substr(s.find_first_of("-") + 1), 0, 100) == false)) {
        throw std::exception("\tOption 'Road.height' invalid; must be 'percentile-XX'.");
      }
    }
  }
  if (n["Terrain"]) {
    if (n["Terrain"]["simplification"]) {
      if (is_string_integer(n["Terrain"]["simplification"].as<std::string>()) == false) {
        throw std::exception("\tOption 'Terrain.simplification' invalid; must be an integer.");
      }
    }
  }
  if (n["Forest"]) {
    if (n["Forest"]["simplification"]) {
      if (is_string_integer(n["Forest"]["simplification"].as<std::string>()) == false) {
        throw std::exception("\tOption 'Forest.simplification' invalid; must be an integer.");
      }
    }
  }
  if (n["Separation"]) {
    if (n["Separation"]["height"]) {
      ...
      if ((s.substr(0, s.find_first_of("-")) != "percentile") ||
        (is_string_integer(s.substr(s.find_first_of("-") + 1), 0, 100) == false)) {
        throw std::exception("\tOption 'Separation.height' invalid; must be 'percentile-XX'.");
      }
    }
  }
  if (n["Bridge/Overpass"]) {
    if (n["Bridge/Overpass"]["height"]) {
      ...
      if ((s.substr(0, s.find_first_of("-")) != "percentile") ||
        (is_string_integer(s.substr(s.find_first_of("-") + 1), 0, 100) == false)) {
        throw std::exception("\tOption 'Bridge/Overpass.height' invalid; must be 'percentile-XX'.");
      }
    }
  }
  //-- 3. input_elevation
  ...
  for (auto it = n.begin(); it != n.end(); ++it) {
    ...
    for (auto it2 = tmp.begin(); it2 != tmp.end(); ++it2) {
      if (is_string_integer(it2->as<std::string>()) == false) {
        throw std::exception("\tOption 'input_elevation.omit_LAS_class' invalid; must be an integer.");
      }
    }
    if ((*it)["thinning"]) {
      if (is_string_integer((*it)["thinning"].as<std::string>()) == false) {
        throw std::exception("\tOption 'input_elevation.thinning' invalid; must be an integer.");
      }
    }
  }
  //-- 4. options
  ...
  if (n["radius_vertex_elevation"]) {
    try {
      ...
      throw std::exception("\tOption 'options.radius_vertex_elevation' invalid.");
    }
  }
  if (n["threshold_jump_edges"]) {
    try {
      ...
      throw std::exception("\tOption 'options.threshold_jump_edges' invalid.");
    }
  }
  //-- 5. output
  n = nodes["output"];
  ...
  if ((format != "OBJ") &&
    ...
    (format != "Shapefile")) {
    throw std::exception("\tOption 'output.format' invalid (OBJ | OBJ-NoID | CityGML | CityGML-IMGeo | CSV-BUILDINGS | Shapefile)");
  }
  return true;
}
