...
void Map3d::get_citygml_multifile(std::string ofname) {
  ...
  for (auto& f : _lsFeatures) {
    ...
    if (ofs.find(filename) == ofs.end()) {
      ofs.emplace(filename, std::wofstream(filename));
...
void Map3d::get_csv_buildings_all_elevation_points(std::wostream &outputfile) {
  outputfile << "id,allzvalues" << std::endl;
  for (auto& p : _lsFeatures) {
    if (p->get_class() == BUILDING) {
      Building* b = dynamic_cast<Building*>(p);
      outputfile << b->get_id().c_str() << ",";
      outputfile << b->get_all_z_values().c_str();
      outputfile << "," << std::endl;
    }
  }
}

void Map3d::get_csv_buildings_multiple_heights(std::wostream &outputfile) {
  //-- ground heights
...
void Map3d::get_obj_per_feature(std::wostream& of, int z_exaggeration) {
  ...
  of << fs.c_str() << std::endl;
}

void Map3d::get_obj_per_class(std::wostream& of, int z_exaggeration) {
  std::unordered_map< std::string, unsigned long > dPts;
  std::string fs;
  for (int c = 0; c < 6; c++) {
    for (auto& p : _lsFeatures) {
