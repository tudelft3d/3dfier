...
void Map3d::get_citygml(std::ofstream &outputfile) {
  ...
  create_citygml_header(outputfile);
  for (auto& f : _lsFeatures) {
    f->get_citygml(outputfile);
  }
...
void Map3d::get_citygml_multifile(std::string outputfilename) {
  ...
  for (auto& f : _lsFeatures) {
    ...
    f->get_citygml(outputfiles[filename]);
  }
...
void Map3d::create_citygml_header(std::ofstream& outputfile) {
    get_xml_header(outputfile);
    get_citygml_namespaces(outputfile);
    outputfile << "<!-- Automatically generated by 3dfier (https://github.com/tudelft3d/3dfier), a software made with <3 by the 3D geoinformation group, TU Delft -->\n";
    outputfile << "<gml:name>my 3dfied map</gml:name>\n";
    outputfile << "<gml:boundedBy>\n";
    outputfile << "<gml:Envelope srsDimension=\"3\" srsName=\"urn:ogc:def:crs:EPSG::7415\">\n";
    outputfile << "<gml:lowerCorner>";
    outputfile << bg::get<bg::min_corner, 0>(_bbox) << " " << bg::get<bg::min_corner, 1>(_bbox) << " 0";
    outputfile << "</gml:lowerCorner>\n";
    outputfile << "<gml:upperCorner>";
    outputfile << bg::get<bg::max_corner, 0>(_bbox) << " " << bg::get<bg::max_corner, 1>(_bbox) << " 100";
    outputfile << "</gml:upperCorner>\n";
    outputfile << "</gml:Envelope>\n";
    outputfile << "</gml:boundedBy>\n";
}

void Map3d::get_csv_buildings(std::ofstream &outputfile) {
  outputfile << "id;roof;floor\n";
